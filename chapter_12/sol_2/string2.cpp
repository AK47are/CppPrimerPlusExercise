...
String operator+(const char *first, String &st) {
  char *result = new char[strlen(first) + st.len + 1];
  strcpy(result, first);
  return String(strcat(result, st.str));
}

String operator+(String &first, String &st) {
  return String(
      strcat(strcpy((new char[first.len + st.len + 1]), first.str), st.str));
}
